 ...
 */
     ...
     IS_USED(MODULE_PSA_CIPHERS_AES_256_CBC) || \
     IS_USED(MODULE_PSA_MAC_HMAC_SHA_256) || \
     IS_USED(MODULE_PSA_SECURE_ELEMENT_ATECCX08A_ECC))
#define CONFIG_PSA_MAX_KEY_SIZE 32
#elif (IS_USED(MODULE_PSA_CIPHERS_AES_192_CBC) || \
       IS_USED(MODULE_PSA_ASYMMETRIC_ECC_P192R1))
#define CONFIG_PSA_MAX_KEY_SIZE 24
#elif (IS_USED(MODULE_PSA_CIPHERS_AES_128_CBC)) || \
      (IS_USED(MODULE_PSA_CIPHERS_AES_128_ECB))
...
#define PSA_CIPHER_DECRYPT_OUTPUT_SIZE(key_type, alg, input_length) \
    ((PSA_BLOCK_CIPHER_BLOCK_LENGTH(key_type) > 1) ? \
     (input_length + (input_length % PSA_BLOCK_CIPHER_BLOCK_LENGTH(key_type))) : \
     input_length)
