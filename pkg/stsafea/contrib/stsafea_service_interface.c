...
{
  (void)Length1;
  /*TODO replace with RIOT version*/
...
int8_t StSafeA_HW_Probe(void *pCtx)
{
  ...
  HwCtx->BusInit    = bus_init;
  HwCtx->BusDeInit  = bus_deinit;
  HwCtx->BusSend    = bus_send;
  HwCtx->BusRecv    = bus_recv;
  HwCtx->CrcInit    = crc_init;
  HwCtx->CrcCompute = crc_comp;
  HwCtx->TimeDelay  = time_delay;
